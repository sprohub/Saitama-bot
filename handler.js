  ...
  try {
    ...
    try {
      ...
      if (chat) {
        ...
        if (!('antiLinkAction' in chat)) chat.antiLinkAction = 'delete'
    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || '')[0])) {
        ...
        if (plugin.rowner && plugin.owner && !(isROwner || isOwner)) {
          fail('owner', m, conn, usedPrefix, command)
          continue
        }
        if (plugin.rowner && !isROwner) {
          fail('rowner', m, conn, usedPrefix, command)
          continue
        }
        ...
        if (plugin.mods && !isMods) {
          fail('mods', m, conn, usedPrefix, command)
          continue
        }
        if (plugin.premium && !isPrems) {
          fail('premium', m, conn, usedPrefix, command)
          continue
        }
        if (plugin.group && !m.isGroup) {
          fail('group', m, conn, usedPrefix, command)
          continue
        } else if (plugin.botAdmin && !isBotAdmin) {
          fail('botAdmin', m, conn, usedPrefix, command)
          continue
        } else if (plugin.admin && !isAdmin) {
          fail('admin', m, conn, usedPrefix, command)
          continue
        }
        if (plugin.private && m.isGroup) {
          fail('private', m, conn, usedPrefix, command)
          continue
        }
        if (plugin.register === true && _user.registered === false) {
          fail('unreg', m, conn, usedPrefix, command)
          continue
        }
