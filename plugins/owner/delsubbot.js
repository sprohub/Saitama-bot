...
const handler = async (m, { conn, args, isROwner, usedPrefix }) => {
  try {
    ...
    if (resultado.ok) {
      ...
      const detalle = [
        !resultado.procesoEliminado ? 'No se pudo detener el proceso en PM2.' : null,
        !resultado.carpetaEliminada ? 'No se pudo borrar la carpeta de sesión.' : null,
        resultado.motivo ? `Motivo: ${resultado.motivo}` : null
      ].filter(Boolean).join(' ')
