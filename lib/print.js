...
export default async function (m, conn = { user: {} }) {
    let _name = m.sender ? conn.getName(m.sender) : '';
    let sender = m.sender
        ? PhoneNumber('+' + m.sender.replace('@s.whatsapp.net', '')).getNumber('international') + (_name ? ' ~' + _name : '')
        : '(sin remitente)';
    let chat = m.chat ? conn.getName(m.chat) : '';
    let img;
    try {
        if (global.opts['img']) {
...
m.mtype ? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : ''
    ...
    if (typeof m.text === 'string' && m.text) {
        ...
        if (m.mentionedJid) for (let user of m.mentionedJid) {
            let mentionName = conn.getName(user) || user.split('@')[0];
    ...
    if (m.messageStubParameters) {
        ...
        for (let jid of m.messageStubParameters) {
            ...
            if (name) lineas.push(chalk.gray(`${phoneNumber} (${name})`))
        }
