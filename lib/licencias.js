...
function normalizarNumero(numero) {
  // Quita el sufijo de dispositivo (ej: "573001234567:25" → "573001234567")
  // ANTES de limpiar dígitos, porque si no, el ":25" se pega al número
  // final y rompe todas las comparaciones.
  return (numero || '').split(':')[0].replace(/[^0-9]/g, '')
}
...
/**
 * Lista solo los grupos cuya licencia fue generada por ESE owner
 * específico (el JID que corrió .codegrupo para crear el código).
 * Así cada owner ve solo a sus propios clientes, no los de otros
 * owners.
 * @returns {Array<{groupId, numero, codigo, expiraEn, activo}>}
 */
export function licenciasDeOwner(ownerJid) {
  ensureStorage()
  const resultado = []
  for (const [groupId, chat] of Object.entries(global.db.data.chats)) {
    const licencia = chat?.licencia
    if (!licencia) continue

    const claveCodigo = (licencia.codigo || '').toUpperCase()
    const entradaCodigo = global.db.data.settings.codigosLicencia[claveCodigo]

    if (entradaCodigo?.creadoPor === ownerJid) {
      resultado.push({ groupId, ...licencia })
    }
  }
  return resultado
}
