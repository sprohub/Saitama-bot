...
console.log(chalk.bold.yellow('   👊 SAITAMA BOT - UN SOLO GOLPE 👊'));
console.log(chalk.bold.cyan('═'.repeat(60)));
console.log(chalk.magenta('   「No soy héroe por diversión... soy héroe porque quiero serlo」'));
console.log(chalk.bold.cyan('═'.repeat(60) + '\n'));
...
async function reconnectSubBot(botPath) {
  console.log(chalk.yellow(`👊 [SAITAMA BOT] Despertando sub-bot: ${path.basename(botPath)}`));
  try {
    const { state: subBotState, saveCreds: saveSubBotCreds } = await useMultiFileAuthState(botPath);

    if (!subBotState.creds.registered) {
      console.warn(chalk.yellow(`⚠️ [SAITAMA BOT] Sub-bot en ${path.basename(botPath)} no está registrado`));
      return;
    }
    ...
    subBotConn.ev.on('connection.update', (update) => {
      ...
      if (connection === 'open') {
        console.log(chalk.green(`✨ [SAITAMA BOT] Sub-bot despertado: ${path.basename(botPath)}`));
        const yaExiste = global.conns.some(c => c.user?.jid === subBotConn.user?.jid);
        if (!yaExiste) {
          global.conns.push(subBotConn);
          console.log(chalk.green(`👊 [SAITAMA BOT] Sub-bot fusionado: ${subBotConn.user?.jid}`));
        }
      } else if (connection === 'close') {
        const reason = new Boom(lastDisconnect?.error)?.output?.statusCode;
        console.error(chalk.red(`💥 [SAITAMA BOT] Sub-bot caído en ${path.basename(botPath)}. Razón: ${reason}`));

        if (reason === DisconnectReason.loggedOut || reason === 401) {
          console.log(chalk.red(`❌ [SAITAMA BOT] Desconexión permanente. Eliminando ${path.basename(botPath)}.`));
          global.conns = global.conns.filter(conn => conn.user?.jid !== subBotConn.user?.jid);
          try {
            rmSync(botPath, { recursive: true, force: true });
            console.log(chalk.green(`✅ [SAITAMA BOT] Sub-bot eliminado: ${botPath}`));
          } catch (e) {
    ...
    console.log(chalk.blue(`👊 [SAITAMA BOT] Manejador asignado a: ${path.basename(botPath)}`));
...
async function startSubBots() {
  ...
  if (!existsSync(rutaJadiBot)) {
    ...
    console.log(chalk.bold.cyan(`📁 [SAITAMA BOT] Carpeta de sub-bots creada: ${rutaJadiBot}`));
  } else {
    console.log(chalk.bold.cyan(`📁 [SAITAMA BOT] Carpeta de sub-bots detectada: ${rutaJadiBot}`));
  }

  const readRutaJadiBot = readdirSync(rutaJadiBot);
  if (readRutaJadiBot.length > 0) {
    const credsFile = 'creds.json';
    console.log(chalk.magenta(`👊 [SAITAMA BOT] Buscando sub-bots... Total: ${readRutaJadiBot.length}`));
    ...
    for (const subBotDir of readRutaJadiBot) {
      ...
      if (statSync(botPath).isDirectory()) {
        ...
        if (readBotPath.includes(credsFile)) {
          console.log(chalk.magenta(`👊 [SAITAMA BOT] Sub-bot detectado en ${subBotDir}. Despertando...`));
          await reconnectSubBot(botPath);
        } else {
          console.log(chalk.yellow(`⚠️ [SAITAMA BOT] Sub-bot latente en ${subBotDir} (sin creds.json)`));
        }
      }
    }
    console.log(chalk.magenta(`✅ [SAITAMA BOT] Proceso de sub-bots completado.`));
  } else {
    console.log(chalk.gray(`🌙 [SAITAMA BOT] No hay sub-bots para despertar.`));
  }
}
...
async function handleLogin() {
  if (conn.authState.creds.registered) {
    console.log(chalk.green('✅ [SAITAMA BOT] Ya registrado.'));
    return;
  }
  ...
  let loginMethod = await question(
    ...
    `║      👊 SAITAMA BOT MODE 👊        ║\n` +
    `╠════════════════════════════════════╣\n` +
    `║ ¿Cómo deseas activar el Un Golpe?  ║\n` +
    `║                                    ║\n` +
  ...
  if (global.db) {
    setInterval(async () => {
      if (global.db.data && global.isDatabaseModified) {
        ...
        global.isDatabaseModified = false;
        console.log(chalk.gray('💾 [SAITAMA BOT] Base de datos guardada'));
      }
...
setInterval(() => {
  ...
  clearTmp();
  console.log(chalk.gray('🧹 [SAITAMA BOT] Limpieza temporal completada'));
}, 180000);

if (typeof global.gc === 'function') {
  setInterval(() => {
    console.log(chalk.gray(`🧠 [SAITAMA BOT] Optimizando poder...`));
    global.gc();
  }, 180000);
} else {
...
console.log(chalk.bold.yellow('   👊 SAITAMA BOT - UN SOLO GOLPE 👊'));
console.log(chalk.bold.cyan('   「La bot está lista para eliminar cualquier amenaza」'));
console.log(chalk.bold.yellow('👊'.repeat(20) + '\n'));
